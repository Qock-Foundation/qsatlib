...
def test_vertices():
    solver = BruteForceSolver()
    g = UndirectedGraph(num_vertices=1)
    u = g.vertex()
    formula = forall(g, u == u)
    print('u formula', formula)
    assert solver.solve(formula)
    g = UndirectedGraph(num_vertices=2)
    u = g.vertex()
    formula = forall(g, u == u)
    print('u2 formula', formula)
    assert not solver.solve(formula)
    g = UndirectedGraph(num_vertices=2)
    u, v = g.vertex(), g.vertex()
    formula = forall(g, u, v, (u == v) | (u != v))
    print('u == v', u == v)
    print('u != v', u != v)
    ...
    g = UndirectedGraph(num_vertices=2)
    u, v = g.vertex(), g.vertex()
    formula = forall(g, u, v, (u == v) | g.has_edge(u, v))
    print('formula2', formula)
    assert not solver.solve(formula)
    g = UndirectedGraph(num_vertices=2)
    u, v = g.vertex(), g.vertex()
    formula = forall(g, u, v, g.has_edge(u, v) | ~g.has_edge(u, v))
    print('formula3', formula)
    assert solver.solve(formula)
